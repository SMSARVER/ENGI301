#https://www.youtube.com/watch?v=8392NJjj8s0 #code from here
import numpy as np
import random
import sys
import math
...
''' START OF MAIN CODE '''
#servo1 = SERVO.setup() --- dont know why this is not working
lcd = LCD.LCD_Display()
...
while True: #game repeates when over
	...
	minimax_depth = 3 #depth how many turns in the future alg looks. max of 5 for PB processing power. min of 1 
	## minimax_depth = getDifficulty() #sets difficulty from a range of 1-4 with button (short press for next, long press for select)
	...
	while not game_over:
		...
		if turn == AI and not game_over:				
			...
			if is_valid_location(board, col):
				...
				##moveToCol(row) #moves stepper to position 0-6 on board
				##releasePiece() #rotates servo 360 deg to release 1 robot game piece SERVO.right()
				##moveToOrigin(row) #moves stepper backwards to origin
